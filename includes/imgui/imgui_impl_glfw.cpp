...
#else
    // Fallback for systems without cursor support
    void*                   MouseCursors[ImGuiMouseCursor_COUNT];
    void*                   LastMouseCursor;
#endif
...
#if GLFW_HAS_CREATECURSOR
            GLFWcursor* cursor = (GLFWcursor*)((bd->MouseCursors[imgui_cursor] ? bd->MouseCursors[imgui_cursor] : bd->MouseCursors[ImGuiMouseCursor_Arrow]));
...
#else
            // Fallback for older GLFW versions or when cursor creation is disabled
